...
def init_args():
    ...
    parser.add_argument("--e2e_model_dir", type=str, default='./xml_model/inference.xml')
...
def create_predictor(args, mode):
    ...
    # OpenVINO support here
        ...
        model_file_path = model_dir
        if not os.path.exists(model_file_path):
            raise ValueError("not find OpenVINO model file {}". format(model_file_path))
        sess = core.compile_model(model=model_file_path, device_name="CPU")
        return sess, sess.input(0), None, None
    else:
        ...
        if args.use_gpu:
            ...
            if gpu_id is None:
                ...
                # )
                print("GPU is not found in current device by nvidia-smi. Please check your device or ignore it if run on jetson.")
